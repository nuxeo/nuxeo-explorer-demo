(function(window) {
    ...
    function NXPlotly() {
        ...
        function hexToRGB(hex) {
            ...
            return "rgba(" + r + ", " + g + ", " + b + ", " + MARKER_OPACITY.DEFAULT + ")";
        }
        ...
        function highlightColor(color, doHighlight) {
            ...
            if (alpha != MARKER_OPACITY.SELECTED) {
        ...
        function getNodeTrace(nxgraph, type, config) {
            ...
            var trace = {
                    ...
                    marker: {
                        ...
                        size: nodes.map(node => nodeWeight(node.weight)), // to be customized on highlight
                        // XXX: color is not shared with hover background in 3D...
                        color: nodes.map(node => hexToRGB(nodeColor(node.category))), // to be changed on selection/highlight
                        opacity: 1,
                        line: {
                            color: 'rgb(50,50,50)',
                            width: 0.5
        ...
        function getEdgeTrace(nxgraph, type, config) {
            ...
            var colors = type ? hexToRGB(edgeColor(type)) : edges.map(edge => hexToRGB(edgeColor(edge.value)));
            //var colors = edges.map((edge, index) => index%2 == 0 ? 'rgba(255, 0, 0, 0.1)': 'rgba(255, 0, 0, 1)');
            ...
            var markers = {
                    ...
                    marker: {
                        ...
                        color: (type ? colors: computeLineMarkersData(colors, nbpoints)), // to be changed on selection/highlight
        ...
        // - udpate opacity on nodes
        // - update width on edges
        ...
        function getUpdateMenus(traces) {
            ...
            // FIXME: use event handlers for all these, to preserve selection status highlighting
            menus.push({
        ...
        function getInitialTraceUpdates(traces) {
            ...
            for (var ti = 0; ti < traces.length; ti++) {
                ...
                if (Array.isArray(trace.marker.color)) {
                    traceupdates[ti] = [...trace.marker.color];
                } else {
                    traceupdates[ti] = trace.x.map(item => trace.marker.color);
                }
            }
        ...
        function performTraceUpdates(lnxgraph, traces, traceupdates) {
            ...
            for (var ti = 0; ti < traces.length; ti++) {
                ...
                Plotly.restyle(lnxgraph.div, {'marker.color': [traceupdates[ti]]}, [ti]);
